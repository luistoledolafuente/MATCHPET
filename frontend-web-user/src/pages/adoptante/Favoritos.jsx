...
import { Loader2, Heart, PawPrint, MapPin, Trash2 } from "lucide-react";
...
export default function Favoritos() {
  ...
  // 1. Cargar la lista de favoritos al entrar
  ...
  // 2. Función para eliminar de favoritos
  const handleRemove = async (animalId) => {
    // Optimismo: Quitamos de la vista inmediatamente para que se sienta rápido
    ...
    try {
      ...
      // Si falla, restauramos la lista original
      setFavoritos(backup);
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center min-h-screen bg-[#FFF7E6]">
        <Loader2 className="animate-spin w-10 h-10 text-[#316B7A] mb-4" />
        <p className="text-[#316B7A]">Cargando tus favoritos...</p>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-[#FFF7E6] p-8 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        {/* --- ESTADO VACÍO --- */}
        {favoritos.length === 0 ? (
          <div className="text-center bg-white p-12 rounded-2xl shadow-sm max-w-2xl mx-auto mt-10">
            ...
            <p className="text-gray-500 mb-8 max-w-md mx-auto">
              ¡Parece que aún no te has enamorado! Explora las mascotas
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {favoritos.map((animal) => (
              <div
                ...
                className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition flex flex-col relative group"
              >
                {/* Botón Eliminar (Visible siempre o en hover) */}
                <button
                  ...
                  <img
                    src={
                      animal.fotos?.[0]
                        ...
                        : "https://placehold.co/400x300/e2e8f0/cbd5e1?text=Sin+Foto"
                    }
                    alt={animal.nombre}
                ...
                <div className="p-5 flex-1 flex flex-col">
                  <div className="flex justify-between items-start mb-2">
                    <h2 className="text-xl font-bold text-[#316B7A]">
                      {animal.nombre}
                    </h2>
                    <span
                  ...
                  <p className="text-sm text-gray-500 mb-4 line-clamp-2">
                    {animal.descripcionPersonalidad ||
                      "Sin descripción disponible."}
                  </p>

                  <div className="flex items-center text-gray-500 text-xs mb-5 mt-auto">
                  ...
                  <Link
                    ...
                    className="block text-center w-full py-2 border border-[#316B7A] text-[#316B7A] font-semibold rounded-lg hover:bg-[#316B7A] hover:text-white transition text-sm"
                  >
                    Ver Detalles
                  </Link>
                </div>
